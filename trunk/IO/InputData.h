...
namespace MultiBoost {
	...
	*/
	...
	public:
		...
		* @date 12/11/2005
		*/
		InputData() : _numColumns(0), _numExamples(0),
			_hasExampleLabel(false), _classInLastColumn(false) {}
		...
		* @date 08/11/2005
		*/
		virtual void load(const string& fileName, eInputType inputType = IT_TRAIN, 
			int verboseLevel = 1);
		...
		* @date 10/11/2005
		*/
		const int  getClass(int idx) const { return _infoData[idx].classIdx; }
		...
		* @date 11/11/2005
		*/
		double   getValue(int idx, int columnIdx) const { return _data[idx][columnIdx]; }
		...
		* @date 14/2/2006
		*/
		string   getLabel(int idx) { return _infoData[idx].label; }
		...
		*/
		...
		int      getNumColumns()  const { return _numColumns; }   //!< Returns the number of columns.
		...
		*/
		...
		void loadFromImage(IplImage *imageData)
		{
			_numColumns = imageData->width * imageData->height;

			double* pDataArray = NULL;

			string tmpFileName = "";
			string tmpClassName = "xxx"; // cannot be one of the class names contained in the shyp file
			...
			for (int j = 0; j < _numColumns; ++j)
				pDataArray[j] = imageData->imageData[j]*65535.;
